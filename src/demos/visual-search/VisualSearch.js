...
const VisualSearch = () => {
  ...
  const onRandomSearch = async () => {
    const data = await getRandomDataFromOnaji();
    setNfts(
      data?.hits.map((nft) => ({
        ...nft,
        image: `${CDN_URL_BASE_PATH}${generateUUID(
  ...
  return (
    <div>
      <Header
        title="Visual Search"
        titleInfo={"Search for NFTs based on visual similarity"}
      />
      <SearchBar
        placeholderText={
          "Search for NFTs based on visuals, colors, narrartives, or any other inspiration"
        }
